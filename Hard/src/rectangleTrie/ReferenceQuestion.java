...
public class ReferenceQuestion {
	...
	private ReferenceWordGroup[] groupList;
	private Trie trieList[];

	public ReferenceQuestion(String[] list) {
		groupList = ReferenceWordGroup.createWordGroups(list);
	 ...
	 */
	public ReferenceRectangle maxRectangle() {
			...
			for (int i = 1; i <= maxWordLength; i++) {
				if (z % i == 0) {
					...
					if (j <= maxWordLength) {
						...
						// j can be created.
						ReferenceRectangle rectangle = makeRectangle(i, j);
						if (rectangle != null) {
							return rectangle;
						}
					}
	 ...
	 */
	private ReferenceRectangle makeRectangle(int length, int height) {
		...
		return makePartialRectangle(length, height, new ReferenceRectangle(length));
	}
	 ...
	 */
	private ReferenceRectangle makePartialRectangle(int l, int h, ReferenceRectangle rectangle) {
		...
		for (int i = 0; i < groupList[l - 1].length(); i++) {
			ReferenceRectangle orgPlus = rectangle.append(groupList[l - 1].getWord(i));
			ReferenceRectangle rect = makePartialRectangle(l, h, orgPlus);
			if (rect != null) {
				return rect;
			}
		}
