...
public class ReferenceRectangle {
	...
	public ReferenceRectangle(int len) {
		this.length = len;
	}
	 ...
	 * that the length and height specified as arguments are consistent with the
	 * array argument's dimensions.)
	 */
	public ReferenceRectangle(int length, int height, char[][] letters) {
	...
	public boolean isComplete(int l, int h, ReferenceWordGroup groupList) {
	...
	/*
	 * If the length of the argument s is consistent with that of this Rectangle
	 * object, then return a Rectangle whose matrix is constructed by appending
	 * s to the underlying matrix. Otherwise, return null. The underlying matrix
	 * of this Rectangle object is /not/ modified.
	 */
	public ReferenceRectangle append(String s) {
		if (s.length() == length) {
			...
			return new ReferenceRectangle(length, height + 1, temp);
		}
		return null;
	}
