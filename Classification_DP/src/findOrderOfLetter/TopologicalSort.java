...
//开始的设置low car 为空格，然后在map 里面随便找一个作为next 然后再找一个做位prev，假设这个是lowest 表示没有incoming 的如果是检查有prev-> next 说明就不是lowest 就标记为lowest为false 然后出来
//如果不是就再找，如果是的话就将lowestchar 变为next 而且也存到L当中。然后break 出来。如果是lowestChar 是有的话，就说明是成功的，就将这个给删除。lowestChar 置为空格。继续。如果出来了但是是空格说明的是里面没有最小的，是有一个环的，就表示可以直接返回的null
//如果完全成功就直接将这个result 返回去就是最后的结果。
